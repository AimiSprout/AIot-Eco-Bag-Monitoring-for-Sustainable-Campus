...
def sort_bag(bag_type):
    ...
    if bag_type == 'paperbag':
        print("[SORT] Paperbag → C1")
...
interpreter = tf.lite.Interpreter(
    model_path='/home/pi/eco_bag_model4.tflite'
)
...
with open('/home/pi/labels4.txt', 'r') as f:
...
def check_bin_full():
    ...
    for bag_type, pin in ir_pins.items():
        ...
        if sensor_value == 0:
            ...
            if bin_was_full[bag_type]:
                db.reference('bin_status').update({
                    bag_type: 'OK'
                })
                bin_was_full[bag_type] = False
                print(f"✅ {bag_type.upper()} bin cleared!")
...
db.reference('machine_status').set({'message': 'System Ready'})
...
try:
    while True:
        ...
        if GPIO.input(IR_BAG) == 0:
            print("\n🎒 Bag detected!")
            lcd_display("Bag Detected!", "Classifying...")
            time.sleep(0.5)  # bagi beg stable dulu
            ...
            if avg_conf >= 70.0:
                ...
                lcd_display(
                    f"Type:{predicted_class[:6].upper()}",
                    f"Total:{total} bags"
                )

                sort_bag(predicted_class)
                update_firebase(predicted_class, bag_count)
                check_bin_full()

                print(f"Bag counts: {bag_count}")

            else:
                print(f"Low confidence: {avg_conf:.1f}% - skip")
                lcd_display("Low Confidence", "Try Again...")
                time.sleep(2)
    ...
    print("\n\nSystem stopped!")
    print(f"Final counts: {bag_count}")
    lcd_display("System Off", "Goodbye!")
    time.sleep(2)
    lcd.clear()
